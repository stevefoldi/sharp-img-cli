...
export async function resolveConfig(cliOptions = {}) {
	...
	// Normalize formats
	if (cliOptions.formats) {
		config.formats = cliOptions.formats;
	}

	// Apply preset
	if (config.preset) {
		const preset = userConfig.presets?.[config.preset] || defaultConfig.presets?.[config.preset];
	...
	// Apply default variants ONLY if not provided
	if (!config.variants || config.variants.length === 0) {
		config.variants = ["800x"];
	}
